...
select_storage() {
  ...
  set +e
if [[ "$TEMPLATE_TYPE" != "dir" ]]; then
  set -e
  error_exit "Template storage must be type: dir"
fi

if [[ ! "$ROOTFS_TYPE" =~ ^(dir|lvmthin|zfspool)$ ]]; then
  set -e
  error_exit "RootFS storage must be dir, lvmthin, or zfspool"
fi
set -e
}
